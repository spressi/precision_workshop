---
title: "Crash Course on Handling Uncertainty"
...
iris %>% 
  ...
  summarize(
    setosa.ci.size = t.test(setosa)$conf.int %>% diff(), # or confintr::se_mean(setosa)
    versicolor.ci.size = t.test(versicolor)$conf.int %>% diff(), # or confintr::se_mean(versicolor)
    diff.ci.size = t.test(setosa, versicolor)$conf.int %>% diff() # or confintr::se_mean_diff(setosa, versicolor)
  )
```

## CIs in `R`: Within-Subjects Design

It works similar with paired t-tests (but the pivoting is slightly different):
...
iris %>% 
  ...
  summarize(
    ...
    diff.ci.size = t.test(setosa, versicolor, paired = TRUE)$conf.int %>% diff() #or confintr::se_mean(setosa - versicolor)
  )
```
...
&rArr; Be transparent! Describe how you calculated your CIs!
...
```{r}
#| echo: true
#| code-line-numbers: "2-6|4|8|5|10"
ostt <-
  fhch2010_summary_wordnaming %>% 
  summarize(
