...
aov2 <-
  fhch2010_summary %>% 
  summarize(
    .by = c(task, stimulus), #for each condition combination
    rt.m = mean(rt), 
    rt.m.low = confintr::ci_mean(rt)$interval[[1]], 
    rt.m.high = confintr::ci_mean(rt)$interval[[2]]) %>% 
  ...
  geom_errorbar(aes(ymin = rt.m.low, ymax = rt.m.high), position = position_dodge(.9)) + #explicitly specify default width = .9
...
Usually, we do not visualize the "raw" (trial-level) data but the subject-level aggregates. \
...
```{r}
fhch2010_summary %>% 
  ggplot(aes(x = task, y = rt, colour = stimulus)) +
  # geom_jitter doesn't dodge the groups; geom_point with position_jitterdodge would lead to 
  # the same result
  geom_jitter(position = position_jitterdodge(dodge.width = .75)) +
  theme(legend.position = "top")
```

## Jittered Points

Combine with ANOVA interaction plot:

```{r}
#| eval: false
# individual data sets for different geoms
ggplot() +
  geom_jitter(
    aes(x = task, y = rt, colour = stimulus),
    data = fhch2010_summary,
    position = position_jitterdodge(dodge.width = .75),
    alpha = .4
  ) +
  geom_errorbar(
    aes(y = rt.m, ymin = rt.m.low, ymax = rt.m.high, x = task, group = stimulus), 
    position = position_dodge(.75), width = .25,
    data = fhch2010_summary %>% 
      summarize(
        .by = c(task, stimulus), #for each condition combination
        rt.m = mean(rt), 
  ...
  geom_point(
    aes(x = task, y = rt.m, colour = stimulus),
...
Check out [Allen et al. (2019)](https://doi.org/10.12688%2Fwellcomeopenres.15191.2) for more advanced versions.
