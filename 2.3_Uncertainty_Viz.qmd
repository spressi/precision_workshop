...
aov3 +
  ...
  geom_errorbar(aes(ymin = rt.m.low, ymax = rt.m.high), position = position_dodge(.9), 
                color = "black", size = 1.125)
```
...
aov3 +
  geom_jitter(
    ...
    position = position_jitter(width = .25, seed = 1337),
  ) +
...
aov3 +
  geom_jitter(
    data = fhch2010_summary %>% rename(rt.m = rt) %>% filter(stimulus == "word"),
    position = position_jitterdodge(jitter.width = .25, dodge.width = .5, seed = 1337),
  ) +
  geom_jitter(
    data = fhch2010_summary %>% rename(rt.m = rt) %>% filter(stimulus == "nonword"),
    position = position_jitterdodge(jitter.width = .25, dodge.width = .5, seed = 1337),
  ) +
  geom_line( #add individual slopes
    data = fhch2010_summary %>% rename(rt.m = rt), 
...
#| output-location: slide
aov3 +
  geom_violin( # experts can try ggbeeswarm::geom_beeswarm()
    data = fhch2010_summary %>% rename(rt.m = rt), #conform to naming in aov3
    #aes(y = rt.m, x = stimulus), #inherited from aov3
    aes(fill = task), #dotplot dots have color = outer border, fill = inner color
    alpha = .5
